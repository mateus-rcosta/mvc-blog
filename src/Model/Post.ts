...
export default class Post extends AbstractModel {
  ...
  public async save(): Promise<Post> {
    ...
    const post = this.id
      ? await Database.prisma.post.update({ where: { id: this.id }, data, include: { user: true,  category: true, tags: { include: { tag: true } } } })
      : await Database.prisma.post.create({ data, include: { user: true,  category: true, tags: { include: { tag: true } } } });
  ...
  public static async create(titulo: string, conteudo: string, userId: number): Promise<Post> {
    const prismaPost = await Database.prisma.post.create({
      ...
      include: { user: true, category: true, tags: { include: { tag: true } } }
    });
